...
$(function(){	
	/** 文字效果 **/
	$('#contents label').live('click',function(){
		$(this).css({
			'color':'#0080FF',
			'font-weight':'bold'
		}).siblings().css({
	...
	$('#contents input:text').live('keyup',function(){
		var val = $(this).val();
		if(val!='' && val!=' '){
	...
	$('#contents textarea').live('keyup',function(e){
	...
	temp1 = $('<div></div>');
	temp2 = $('<div></div>');
	temp1.load('ajax/part1.html').appendTo($('#contents form'));
	temp2.load('ajax/part2.html').appendTo($('#contents form'));
	...
	$(window).scroll(function(){
		...
		if(st+windowH == conH){
			...
			if(html.load(page)){
				html.load(page).appendTo($('#contents form'));
				pageNow +=1;
			}
			/** 提交答案 **/
